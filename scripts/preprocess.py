...
def get_training_and_testing_sets(train, file_list):
        ...
        final_val = training[split_index:]
        return final_training, final_testing, final_val
...
def imgaug(args):
    ...
    imageio.imwrite(os.path.join(root, os.path.splitext(filename)[0] + '.jpg'), img) #convert the current image in B into a jpg from png
    ...
    augseq = iaa.Sequential(
        [
            ...
            sometimes(iaa.Affine(rotate=(-3, 3))),
    ...
    img = ia.imresize_single_image(img, (2048, 2048))
    ...
    for batch in augseq.augment_batches(batches, background=False):
        for img in batch.images_aug:
            ...
            imageio.imwrite(os.path.join(fold_A,root.rsplit('/', 1)[-1], os.path.splitext(filename)[0] + '_' + str(count) + '.jpg'), img)
            print('image saved as: ' + os.path.join(fold_A,root.rsplit('/', 1)[-1],t + '.jpg'))
            count +=1
...
def main():
    ...
    training, testing, val = get_training_and_testing_sets(args.train, randomized)
    count = 0
    for f in training:
            shutil.copy(args.raw_data + f, args.fold_B + "/train/" + "IMG_" + str(count) + ".png")
            count += 1
    for f in testing:
            shutil.copy(args.raw_data + f, args.fold_B + "/test/" + "IMG_" + str(count) + ".png")
            count +=1
    for f in val:
            shutil.copy(args.raw_data + f, args.fold_B + "/val/" + "IMG_" + str(count) + ".png")
            count +=1
    ...
    print('Total preprocessing took {:.3f} s'.format((time.clock() - total)*1000.0))
